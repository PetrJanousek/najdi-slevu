"""Baseline parser tests for Billa leaflet PDFs.

These tests require:
1. An Billa PDF fixture at tests/fixtures/leaflets/billa/<yyyy-mm-dd>.pdf
2. A hand-curated golden file at tests/fixtures/expected/billa.json
...
To generate the golden draft once you have a PDF:
    python scripts/dump_parser_output.py billa > tests/fixtures/expected/billa.json
Then hand-curate billa.json and re-run pytest.
"""
import json
import math
...
def test_billa_parser_returns_enough_discounts(sample_pdf):
    """Parser extracts at least MIN_DISCOUNTS items from an Billa leaflet."""
    pdf_path = sample_pdf("billa")
...
def test_billa_parser_no_impossible_rows(sample_pdf):
...
def test_billa_parser_dates_plausible(sample_pdf):
...
def test_billa_parser_golden_recall(sample_pdf):
    """≥90% of hand-curated golden items are found in parser output."""
    pdf_path = sample_pdf("billa")
    golden = _load_golden("billa")
