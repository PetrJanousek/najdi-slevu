"""Baseline parser tests for Kaufland leaflet PDFs.

These tests require:
1. An Kaufland PDF fixture at tests/fixtures/leaflets/kaufland/<yyyy-mm-dd>.pdf
2. A hand-curated golden file at tests/fixtures/expected/kaufland.json
...
To generate the golden draft once you have a PDF:
    python scripts/dump_parser_output.py kaufland > tests/fixtures/expected/kaufland.json
Then hand-curate kaufland.json and re-run pytest.
"""
import json
import math
...
def test_kaufland_parser_returns_enough_discounts(sample_pdf):
    """Parser extracts at least MIN_DISCOUNTS items from an Kaufland leaflet."""
    pdf_path = sample_pdf("kaufland")
...
def test_kaufland_parser_no_impossible_rows(sample_pdf):
...
def test_kaufland_parser_dates_plausible(sample_pdf):
...
def test_kaufland_parser_golden_recall(sample_pdf):
    """≥90% of hand-curated golden items are found in parser output."""
    pdf_path = sample_pdf("kaufland")
    golden = _load_golden("kaufland")
