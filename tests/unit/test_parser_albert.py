"""Baseline parser tests for Albert leaflet PDFs.

These tests require:
1. An Albert PDF fixture at tests/fixtures/leaflets/albert/<yyyy-mm-dd>.pdf
2. A hand-curated golden file at tests/fixtures/expected/albert.json
...
To generate the golden draft once you have a PDF:
    python scripts/dump_parser_output.py albert > tests/fixtures/expected/albert.json
Then hand-curate albert.json and re-run pytest.
"""
import json
import math
...
def test_albert_parser_returns_enough_discounts(sample_pdf):
    """Parser extracts at least MIN_DISCOUNTS items from an Albert leaflet."""
    pdf_path = sample_pdf("albert")
...
def test_albert_parser_no_impossible_rows(sample_pdf):
...
def test_albert_parser_dates_plausible(sample_pdf):
...
def test_albert_parser_golden_recall(sample_pdf):
    """≥90% of hand-curated golden items are found in parser output."""
    pdf_path = sample_pdf("albert")
    golden = _load_golden("albert")
