"""Baseline parser tests for Penny leaflet PDFs.

These tests require:
1. An Penny PDF fixture at tests/fixtures/leaflets/penny/<yyyy-mm-dd>.pdf
2. A hand-curated golden file at tests/fixtures/expected/penny.json
...
To generate the golden draft once you have a PDF:
    python scripts/dump_parser_output.py penny > tests/fixtures/expected/penny.json
Then hand-curate penny.json and re-run pytest.
"""
import json
import math
...
def test_penny_parser_returns_enough_discounts(sample_pdf):
    """Parser extracts at least MIN_DISCOUNTS items from an Penny leaflet."""
    pdf_path = sample_pdf("penny")
...
def test_penny_parser_no_impossible_rows(sample_pdf):
...
def test_penny_parser_dates_plausible(sample_pdf):
...
def test_penny_parser_golden_recall(sample_pdf):
    """≥90% of hand-curated golden items are found in parser output."""
    pdf_path = sample_pdf("penny")
    golden = _load_golden("penny")
